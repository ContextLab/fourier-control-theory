...
- **Arm tab** — an animated chain of links (the "puppet") next to its amplitude/power
  ...
  them at the shoulder, elbow, wrist, and knuckles (skin only renders for presets whose
  components carry bone labels, i.e. the human arm). In this human-arm view, a plain drag
  rotates a bone (its length stays fixed and the joint follows the pointer's direction),
  while shift-drag also changes its length, landing the joint exactly under the pointer.
  With the skin toggled off, a plain drag does the full move (length and phase together).
  The Control tab shows the same arm as a read-only display; drag joints on the Arm tab.
  Drag a spectrum stem vertically to change its amplitude, horizontally to change its
  frequency (snapped to the nearest integer). Hold Alt/Option while dragging, drag the
  phase ring around a selected stem's head, or scroll the wheel over it, to change phase
  instead. Double-click empty space to add a component, shift-drop one stem onto another
  at the same frequency to merge them, or select a stem and press Delete/Backspace to
  remove it.
